...
def train_weight_models(features: pd.DataFrame, cfg: dict) -> ModelBundle:
    ...
    complete = train.dropna(subset=FEATURES + ["target_next_return"]).copy()
    if len(complete) < max(30, min(window, 60)):
        raise ValueError(f"Insufficient complete real observations for training: {len(complete)}")
    X, y = complete[FEATURES], complete["target_next_return"]

    rf = RandomForestRegressor(n_estimators=300, min_samples_leaf=5, random_state=cfg["app"]["random_seed"]).fit(X, y)
    lasso = Pipeline([
...
def short_forecast(features: pd.DataFrame, bundle: ModelBundle, days: int = 14) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"SARIMAX forecast failed; report not generated: {exc}") from exc

    latest = features.dropna(subset=FEATURES).tail(1)
    if latest.empty:
        raise ValueError("No complete real feature vector available for forecast")
...
def medium_forecast(features: pd.DataFrame, bundle: ModelBundle, days: int = 90) -> dict:
    ...
    latest = features.dropna(subset=FEATURES).tail(1)
    if latest.empty:
        raise ValueError("No complete real feature vector available for medium forecast")
    latest_x = latest[FEATURES]
