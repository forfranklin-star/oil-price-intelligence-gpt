...
class ChinaDieselProvider:
    ...
    def _fetch_longzhong(self) -> SourceResult:
        ...
        url = cfg.get("url", "https://www.oilchem.net/seo-445-7.html")
        r = self._get(url)
        ...
        if sum(len(x) for x in frames) < target_rows and cfg.get("backfill_public_articles", True):
            ...
            for page in range(1, max_pages + 1):
                try:
                    lr = self._get(archive_tmpl.format(page=page))
                    ...
                    for a in soup.find_all("a", href=True):
                        ...
                        if "成品油" not in title:
                            continue
                        # Prefer analytical pages likely to expose the China price table.
                        if not any(k in title for k in ["早间", "提示", "预期", "价格", "汽柴"]):
                            continue
                    ...
                    self.diagnostics.append({"source": "longzhong_archive", "status": "page_failed", "detail": str(exc), "page": page})
                if len(links) >= max_articles:
                    break
