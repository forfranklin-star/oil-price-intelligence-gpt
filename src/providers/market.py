...
class MarketProvider:
    ...
    def _fetch_fred_oil(self, series_id: str, days: int) -> pd.DataFrame:
        """Real EIA spot series mirrored by FRED; no FRED API key is required."""
        url = self.cfg["data"]["fred_csv_base"]
        r = polite_get(
            self.session, url,
        ...
        if len(df.columns) < 2:
            raise MarketDataError(f"FRED {series_id}: malformed response")
        df = df.iloc[:, :2]
        df.columns = ["date", "price"]
        df["date"] = pd.to_datetime(df["date"], errors="coerce")
        df["price"] = pd.to_numeric(df["price"], errors="coerce")
        cutoff = pd.Timestamp.now().normalize() - pd.Timedelta(days=days)
        df = df[df["date"] >= cutoff].dropna()
    ...
    def history(self, symbol: str, days: int | None = None) -> tuple[pd.DataFrame, str]:
        ...
        errors = []
        # Multiple Yahoo edge servers: same real series, never blended.
        for base in self.cfg["data"].get("yahoo_chart_bases", [self.cfg["data"]["yahoo_chart_base"]]):
            old = self.cfg["data"]["yahoo_chart_base"]
            self.cfg["data"]["yahoo_chart_base"] = base
            try:
                return self._fetch_yahoo(symbol, days), f"yahoo_chart:{base.split('//')[1].split('/')[0]}"
            except Exception as exc:
                errors.append(f"{base}: {exc}")
            finally:
                self.cfg["data"]["yahoo_chart_base"] = old
        # EIA official API, if a key is configured.
        ...
        # FRED publishes the same EIA spot series as a public CSV mirror.
        fred_id = self.cfg["symbols"].get("fred_market", {}).get(logical or "")
        if fred_id:
            try:
                return self._fetch_fred_oil(fred_id, days), f"fred_eia_spot:{fred_id}"
            except Exception as exc:
                errors.append(f"FRED: {exc}")
        raise MarketDataError(f"No valid real data for {symbol}: {' | '.join(errors)}")
