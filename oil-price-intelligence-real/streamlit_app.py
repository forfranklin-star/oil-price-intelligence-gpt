...
if not files:
    st.error("尚无报告。请先运行：python -m src.pipeline")
    st.stop()
...
c3.metric("国内柴油", f"¥{latest['china_diesel']:.0f}/吨", help="若未接入真实国内数据源，则为透明标注的代理序列")
...
st.write(f"两周代理预测：¥{dsf['mean']:.0f}/吨（{dsf['low']:.0f}–{dsf['high']:.0f}）；三个月代理预测：¥{dmf['mean']:.0f}/吨（{dmf['low']:.0f}–{dmf['high']:.0f}）。")
